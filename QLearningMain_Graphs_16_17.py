...
def main():
    ...
    SAVE_AGENT_ONE = True
    SAVE_AGENT_TWO = True
    SAVE_AGENT_THREE = True

    RUN_AGENT_ONE = True
    RUN_AGENT_TWO = True
    RUN_AGENT_THREE = True

    LOAD_AGENT_ONE = False
    LOAD_AGENT_TWO = False
    LOAD_AGENT_THREE = False
    ## ------------------
    ...
    if SAVE_AGENT_ONE == True:
        with open('PointDictBasic_Graph_16_17.pickle', 'wb') as handle:
            pickle.dump(point_dict_basic, handle, protocol=pickle.HIGHEST_PROTOCOL)
            handle.close()

        with open('PointListBasic_Graph_16_17.pickle', 'wb') as handle:
            pickle.dump(point_list_basic, handle, protocol=pickle.HIGHEST_PROTOCOL)
            handle.close()

        with open('SpecifierBasic_Graph_16_17.pickle', 'wb') as handle:
            pickle.dump(specifier_basic, handle, protocol=pickle.HIGHEST_PROTOCOL)
            handle.close()

        with open('PointDictConsecutive_Graph_16_17.pickle', 'wb') as handle:
            pickle.dump(point_dict_consecutive, handle, protocol=pickle.HIGHEST_PROTOCOL)
            handle.close()

        with open('PointListConsecutive_Graph_16_17.pickle', 'wb') as handle:
            pickle.dump(point_list_consecutive, handle, protocol=pickle.HIGHEST_PROTOCOL)
            handle.close()

        with open('SpecifierConsecutive_Graph_16_17.pickle', 'wb') as handle:
            pickle.dump(specifier_consecutive, handle, protocol=pickle.HIGHEST_PROTOCOL)
            handle.close()

    if LOAD_AGENT_ONE == True:
        try:
            with open('PointDictBasic_Graph_16_17.pickle', 'rb') as handle:
                point_dict_basic = pickle.load(handle)
                handle.close()

            with open('PointListBasic_Graph_16_17.pickle', 'rb') as handle:
                point_list_basic = pickle.load(handle)
                handle.close()

            with open('SpecifierBasic_Graph_16_17.pickle', 'rb') as handle:
                specifier_basic = pickle.load(handle)
                handle.close()

            with open('PointDictConsecutive_Graph_16_17.pickle', 'rb') as handle:
                point_dict_consecutive = pickle.load(handle)
                handle.close()

            with open('PointListConsecutive_Graph_16_17.pickle', 'rb') as handle:
                point_list_consecutive = pickle.load(handle)
                handle.close()

            with open('SpecifierConsecutive_Graph_16_17.pickle', 'rb') as handle:
